...
class CategoriaProyecto(models.Model):
    ...
    descripcion = models.TextField(
        blank=True,
        help_text=(
            "Descripción breve de la categoría, ej: Visión por Computadora, "
...
class StackProyecto(models.Model):
    ...
    descripcion = models.TextField(
        blank=True,
        help_text="Descripción breve del stack tecnológico, ej: Python, TensorFlow, AWS, etc.",
    )
...
class SectorProyecto(models.Model):
    ...
    descripcion = models.TextField(
        blank=True,
        help_text="Descripción breve del sector, ej: Finanzas, Salud, Retail, etc.",
    )
...
class MetricaObjetivo(models.Model):
    nombre = models.CharField(
        max_length=100,
        unique=True,
        help_text="Ej: AUC-ROC, RMSE, Accuracy, F1-Score",
    )
    descripcion_metrica = models.TextField(
        blank=True,
        help_text="Breve explicación de qué mide esta métrica",
    )
...
class Proyecto(models.Model):
    ...
    imagen_url = models.URLField(
        ...
        null=True,
        blank=True,
        help_text="Pega aquí la URL de la imagen del proyecto",
    )
    ...
    url_notebook = models.URLField(max_length=255, blank=True)
